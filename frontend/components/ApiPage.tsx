...
const ApiPage: React.FC<ApiPageProps> = ({ theme, toggleTheme }) => {
  ...
  const handleFileSelect = async (files: File[]) => {
    if (files.length > 1) {
      setError("Please upload only one file to test the API.");
          ...
          <div className="w-full max-w-4xl p-8 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg text-left">
            ...
            <div className="space-y-6 text-sm sm:text-base text-slate-600 dark:text-slate-400">
              <div>
                ...
                <p className="mb-1">
                  You can programmatically upload images by sending a POST
                  request to our API endpoint. The request must be of type{" "}
                  <code className="bg-slate-200 dark:bg-slate-700 p-1 rounded-md text-xs">
                    multipart/form-data
                  </code>{" "}
                  and include a{" "}
                  <code className="bg-slate-200 dark:bg-slate-700 p-1 rounded-md text-xs">
                ...
                <pre className="bg-slate-200 dark:bg-slate-900 p-3 rounded-md text-slate-800 dark:text-slate-300 text-sm overflow-x-auto">
                  <code>POST https://api.gmbr.web.id/upload</code>
                </pre>
              </div>
              <div>
                <h2 className="text-xl font-semibold text-slate-800 dark:text-slate-100 mb-2">
              ...
              <div>
                ...
                <p>
                  Upon successful upload via the interactive tester below, you
                  will receive a JSON object containing a shareable link to our
                  viewer and a raw URL for direct image access.
                </p>
              </div>
